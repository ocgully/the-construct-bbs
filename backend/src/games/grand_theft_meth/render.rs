...
/// Format cents as dollars: 123456 -> "$1,234" (no cents)
...
/// Render the game header with title art (80 chars wide max)
...
/// Abbreviate city name for status bar
fn abbreviate_city(city_key: &str) -> &'static str {
    match city_key {
        "nyc" => "NYC",
        "miami" => "Miami",
        "london" => "London",
...
pub fn render_status_bar(state: &GameState, prices: &HashMap<String, i64>) -> String {
    ...
    // Location info - use abbreviated city name
    let city = abbreviate_city(&state.city);
    ...
    w.set_fg(Color::LightCyan);
    w.write_str(&format!("Actions: {}", state.actions_remaining));

    // Show high status if applicable
    if state.high_tier > 0 {
        w.set_fg(Color::DarkGray);
        w.write_str(" | ");
        w.set_fg(Color::LightMagenta);
        let high_text = match state.high_tier {
            1 => "Buzzed",
            2 => "High",
            _ => "WASTED",
        };
        w.write_str(high_text);
    }
    w.writeln("");
...
pub fn render_main_menu(state: &GameState, prices: &HashMap<String, i64>) -> String {
    ...
    // Show last message if any (combat results, etc.)
    ...
    // Use drugs option (only if has inventory)
...
/// Render trade screen with location-specific inventory
pub fn render_trade(state: &GameState, prices: &HashMap<String, i64>, mode: &TradeMode) -> String {
    ...
    match mode {
        TradeMode::Menu => {
            ...
            // Show cancel option if there's a pending transaction
            if let Some(ref pending) = state.pending_transaction {
                w.writeln("");
                w.set_fg(Color::Yellow);
                let action = if pending.is_purchase { "purchase" } else { "sale" };
                let name = get_commodity(&pending.commodity).map(|c| c.name).unwrap_or("???");
                w.writeln(&format!("  Last {}: {} {} for {}", action, pending.quantity, name, format_money(pending.total_cost)));
                w.set_fg(Color::LightRed);
                w.write_str("    [C] ");
                w.set_fg(Color::White);
                w.writeln("Cancel transaction (costs 1 action)");
            }
        ...
        TradeMode::Buying | TradeMode::Selling => {
            ...
            w.bold();
            if is_buying {
                w.writeln(&format!("  BUY - {} slots free", capacity_left));
            } else {
                w.writeln("  SELL - Green = Profit, Red = Loss");
            }
            w.reset_color();
            w.writeln("");

            // Get local shop inventory
            let shop = get_shop_inventory(&state.city, &state.location);

            // Header - different for buy vs sell
            w.set_fg(Color::DarkGray);
            if is_buying {
                w.writeln("     #   Product        Price          Owned");
            } else {
                w.writeln("     #   Product        Price      Cost   Owned");
            }
            w.writeln(&format!("    {}", "\u{2500}".repeat(55)));
            w.reset_color();

            for (i, (commodity_key, _)) in shop.iter().enumerate() {
                let price = prices.get(*commodity_key).copied().unwrap_or(0);
                let owned = state.get_quantity(*commodity_key);
                let name = get_commodity(commodity_key).map(|c| c.name).unwrap_or("???");

                // Display number: 1-9, then 0 for 10
                let display_num = if i + 1 == 10 { 0 } else { i + 1 };

                if is_buying {
                    // BUYING: color by affordability
                    let can_afford = price <= state.cash && capacity_left >= 1;

                    if can_afford {
                        w.set_fg(Color::LightCyan);
                        w.write_str(&format!("    [{:1}] ", display_num));
                        w.set_fg(Color::White);
                        w.write_str(&format!("{:<14}", name));
                        w.set_fg(Color::LightGreen);
                    ...
                    // SELLING: color by profit/loss
                    if owned > 0 {
                        // Get lowest cost (what we'll sell first)
                        let cost_basis = state.get_lowest_cost(*commodity_key).unwrap_or(0);
                        let is_profit = price >= cost_basis;
                        ...
                        // Price colored by profit/loss
                        if is_profit {
                            w.set_fg(Color::LightGreen);
                        } else {
                            w.set_fg(Color::LightRed);
                        }
                        w.write_str(&format!("{:>9}", format_money(price)));

                        // Show cost basis
                        w.set_fg(Color::DarkGray);
                        w.write_str(&format!("  @{:>7}", format_money(cost_basis)));

                        w.set_fg(Color::LightGray);
                        w.writeln(&format!("  {:>4}", owned));
                    } else {
                        // Don't own any - grey out
                        w.set_fg(Color::DarkGray);
                        w.write_str(&format!("    [{:1}] ", display_num));
                        w.write_str(&format!("{:<14}", name));
                        w.write_str(&format!("{:>9}", format_money(price)));
...
pub fn render_hospital(state: &GameState, prices: &HashMap<String, i64>, is_mob_doctor: bool) -> String {
    ...
    // Detox option if player is high
    if state.high_tier > 0 {
        let detox_cost = if is_mob_doctor { 25000 } else { 20000 };
        w.set_fg(Color::LightCyan);
        w.write_str("    [D] ");
        w.set_fg(Color::White);
        let high_desc = match state.high_tier {
            1 => "Buzzed",
            2 => "High",
            _ => "WASTED",
        };
        w.writeln(&format!("Detox ({}) - {}", high_desc, format_money(detox_cost)));
    }
...
/// Render use drugs screen
pub fn render_use_drugs(state: &GameState, prices: &HashMap<String, i64>) -> String {
    let mut w = AnsiWriter::new();
    ...
    w.set_fg(Color::Yellow);
    w.bold();
    w.writeln("  SAMPLE YOUR WARES");
    w.reset_color();
    w.set_fg(Color::LightGray);
    w.writeln("  Warning: Some drugs have side effects...");
    w.writeln("");

    // Get owned commodities
    let owned: Vec<(&String, u32)> = state.inventory_lots
        .iter()
        .map(|(k, lots)| (k, lots.iter().map(|l| l.quantity).sum::<u32>()))
        .filter(|(_, qty)| *qty > 0)
        .collect();

    if owned.is_empty() {
        w.set_fg(Color::DarkGray);
        w.writeln("  Your coat is empty.");
    } else {
        // Header
        w.set_fg(Color::DarkGray);
        w.writeln("     #   Drug           Qty   Effect");
        w.writeln(&format!("    {}", "\u{2500}".repeat(50)));
        w.reset_color();

        for (i, (commodity_key, qty)) in owned.iter().enumerate() {
            let name = get_commodity(commodity_key).map(|c| c.name).unwrap_or("???");
            let commodity = get_commodity(commodity_key);
            ...
            if let Some(c) = commodity {
                ...
                if c.action_boost {
                    effects.push("+actions");
                }
                if c.addictive {
                    effects.push("addictive");
                }
                // Health hints
                match commodity_key.as_str() {
                    "krokodil" => effects.push("dangerous"),
                    "fentanyl" => effects.push("risky"),
                    "heroin" => effects.push("harmful"),
                    "weed" => effects.push("relaxing"),
                    "acid" | "dmt" | "bathsalts" => effects.push("trippy++"),
                    "mdma" => effects.push("euphoric"),
                    "cocaine" => effects.push("energy"),
                    _ => {}
                }
                // High effect hints (trippy++ = +2 high, most others = +1)
                if !matches!(commodity_key.as_str(), "weed" | "ludes" | "tidepods" | "acid" | "dmt" | "bathsalts") {
                    effects.push("high+");
                }
    ...
    // Show current addiction levels if any
    let addicted: Vec<_> = state.addiction.iter()
        .filter(|(_, &level)| level > 0)
        .collect();

    if !addicted.is_empty() {
        w.writeln("");
        w.set_fg(Color::LightRed);
        w.bold();
        ...
        for (drug, level) in addicted {
            let name = get_commodity(drug).map(|c| c.name).unwrap_or("???");
            let severity = if *level >= 50 {
                "SEVERE"
            } else if *level >= 25 {
                "Moderate"
            } else if *level >= 10 {
            ...
            w.set_fg(if *level >= 25 { Color::LightRed } else { Color::Yellow });
            w.writeln(&format!("{} ({})", severity, level));
        }
    }
