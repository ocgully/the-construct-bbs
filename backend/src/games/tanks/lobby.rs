//! Lobby and matchmaking system for Tanks
//!
...
use super::data::{MAX_PLAYERS, MIN_PLAYERS};
...
/// Main lobby manager for all Tanks games
pub struct TanksLobby {
    pub lobbies: HashMap<u64, GameLobby>,
    pub active_games: HashMap<u64, TankGame>,
...
impl TanksLobby {
    ...
    pub fn start_game(&mut self, user_id: i64) -> Result<u64, &'static str> {
        ...
        let terrain_seed = rand::thread_rng().gen::<u64>();
        let mut game = TankGame::new(game_id, lobby.host_user_id, terrain_seed);

        for player in lobby.players {
            let _ = game.add_tank(player.user_id, player.handle);
        }
    ...
    pub fn get_player_game(&self, user_id: i64) -> Option<&TankGame> {
        let game_id = self.player_games.get(&user_id)?;
        self.active_games.get(game_id)
    }

    /// Get a player's current game (mutable)
    pub fn get_player_game_mut(&mut self, user_id: i64) -> Option<&mut TankGame> {
        let game_id = *self.player_games.get(&user_id)?;
    ...
    pub fn list_public_lobbies(&self) -> Vec<(u64, usize, usize)> {
        self.lobbies.iter()
            ...
            .map(|(id, lobby)| (*id, lobby.player_count(), MAX_PLAYERS))
            .collect()
    }
    ...
    pub fn cleanup_game(&mut self, game_id: u64) {
        if let Some(game) = self.active_games.remove(&game_id) {
            for tank in game.tanks.values() {
                self.player_games.remove(&tank.user_id);
            }
        }
    }

    /// Tick all active games for turn timers
    pub fn tick_all(&mut self) -> Vec<(u64, bool)> {
        let mut updates = Vec::new();

        for (id, game) in self.active_games.iter_mut() {
            // Check turn timeout
            if let Some(started) = game.turn_started_at {
                if started.elapsed().as_secs() >= super::data::TURN_TIME_SECS {
                    // Time's up, auto-advance
                    game.next_turn();
                    updates.push((*id, true));
                }
            }
        }
...
mod tests {
    ...
    fn test_create_public_lobby() {
        let mut manager = TanksLobby::new();
    ...
    fn test_start_game() {
        ...
        // Set player 2 ready
        if let Some(lobby) = manager.lobbies.get_mut(&1) {
            lobby.set_ready(2, true);
        }

        let result = manager.start_game(1);
        assert!(result.is_ok());
        assert!(manager.active_games.contains_key(&1));
        assert!(manager.lobbies.is_empty());
    }
