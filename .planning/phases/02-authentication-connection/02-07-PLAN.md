...
<task type="auto">
  ...
  <action>
This task is about fixing any integration gaps between Plans 01-06. The individual components should already compile, but they need to work together as a cohesive flow.
...
5. **Authenticated state** (session.rs):
   ...
   - Add a profile command: when user types "profile" or "p" at main menu, render their profile card
...
7. **Main menu update** (welcome_art.rs):
   ...
   - Update welcome_art or create a new function that takes user info:
     ...
     Enter selection:
     ```
   - Add Profile option (P) to main menu
...
Then start server with `cd C:/Git/bbs/backend && cargo run` and verify it starts without errors.
  ...
  <done>Complete session lifecycle compiles and starts. Frontend connect flow, backend ceremony, login, service routing, and disconnect all wired together. Profile accessible from main menu.</done>
</task>
