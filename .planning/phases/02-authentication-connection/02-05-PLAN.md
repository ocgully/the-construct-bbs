...
<task type="auto">
  ...
  <action>
1. Update `backend/src/websocket/session.rs` to manage the full pre-login auth flow:
   ...
   **Modify on_connect():**
   - After connection ceremony completes successfully, set auth_state to Login(LoginFlow::new())
   - Render login header using render_login_header() with tagline from config, node counts from node_manager
   - Send the first login prompt

   **Modify handle_input() for auth routing:**
   ...
   - If Login(flow): handle character-by-character input:
     ...
     - Process LoginResult:
       ...
       - Success: transition to Authenticated state, update node_manager with real user info (release placeholder, assign with real handle), send session token to frontend as JSON: `{"type":"session","token":"..."}`, display welcome-back message: "Welcome back, {handle}! Last login: {date}", show main menu (call show_main_menu)
   ...
   **Important input handling change:**
   ...
   - In Authenticated state: the existing behavior works (services expect complete lines since the terminal onData sends each keypress but services treat each keystroke as input -- actually looking more carefully, services receive each keystroke too)

   Actually, reviewing the session code: handle_input receives whatever the client sends. With xterm.js onData, this is typically single characters. The current service flow processes each keystroke through handle_input. So the existing pattern already works character-by-character. We just need the Login/Registration flows to also work character-by-character.

2. Update `frontend/src/websocket.ts`:
...
3. **Welcome-back message format** (shown after successful login):
   ```
   ────────────────────────────────────────────────────────────────
...
- Session token sent to frontend as JSON, stored in localStorage
