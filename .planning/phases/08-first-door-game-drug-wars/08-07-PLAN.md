...
must_haves:
  truths:
    - "Gang relations can be improved via tribute or quests"
    - "Delivery quests provide rewards and gang favor"
    - "Story quest progresses through 15 steps across cities"
  artifacts:
    - path: "backend/src/game/quest.rs"
      provides: "Quest system with deliveries and story progression"
      exports: ["generate_delivery_quest", "complete_delivery", "advance_story", "STORY_STEPS"]
  key_links:
    - from: "backend/src/game/quest.rs"
      to: "backend/src/game/state.rs"
      via: "modifies quest_state"
      pattern: "state\\.quest_state"
---

<objective>
Implement gang relations and quest systems.

Purpose: Gangs control territory and require management. Quests provide goals and narrative. Story quest gives players a reason to explore and creates replay value.
Output: Complete gang relation system, delivery quests, and 15-step story quest.
</objective>
...
<task type="auto">
  <name>Task 1: Create quest module with delivery quests and story</name>
  <files>backend/src/game/quest.rs, backend/src/game/mod.rs</files>
  <action>
Create backend/src/game/quest.rs with quest logic.

```rust
use rand::prelude::*;
use crate::game::{GameState, DeliveryQuest, CITIES, COMMODITIES, GANGS, get_city, get_gang};
...
pub fn pay_tribute(state: &mut GameState, gang_key: &str) -> Result<String, String> {
    ...
    if state.cash < gang.tribute_cost {
        return Err(format!(
            "The {} require {} tribute.",
            gang.name,
            super::render::format_money(gang.tribute_cost)
        ));
    }
...
Update backend/src/game/mod.rs to add `pub mod quest; pub use quest::*;`
  </action>
  <verify>
...
15-step story quest with narrative and rewards.
Expired deliveries hurt gang relations.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add quest screen rendering and handlers</name>
  <files>backend/src/game/render.rs, backend/src/game/screen.rs</files>
  <action>
Add quest screen render function to render.rs:
...
Update handle_quest in screen.rs:
        ...
        "S" => {
            if can_complete_story_step(&self.state, &self.prices) {
                match complete_story_step(&mut self.state, &self.prices) {
                    Ok((title, reward)) => {
                        // Could show completion message, for now just continue
...
Run `cargo check` - compiles without errors.
  ...
  <done>
Quest screen shows story progress, deliveries, and gang relations.
Story steps can be completed when requirements are met.
Gang relation colors indicate status.
  </done>
</task>
...
Quest system integrates with game flow.
Story progresses through 15 steps.
</verification>

<success_criteria>
- Gang tribute improves relations
- Delivery quests generated with random parameters
- Deliveries complete at destination and award rewards
- Story quest has 15 steps with narrative
- Story requirements include location, commodities, net worth
- Story completion awards "Kingpin" status
</success_criteria>

<output>
After completion, create `.planning/phases/08-first-door-game-drug-wars/08-07-SUMMARY.md`
</output>
