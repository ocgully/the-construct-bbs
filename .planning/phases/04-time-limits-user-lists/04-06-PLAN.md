...
1. Add new fields to the Session struct:
   ...
   ```
   Initialize all to None/false in Session::new().
...
5. Handle timeout -- poll the expired flag from SessionTimer (created in Plan 02 with is_expired() method):

   At the start of handle_authenticated_input (or handle_input), add:
   ```rust
...
6. Handle time bank withdrawal at 1-minute warning -- poll the low_time flag from SessionTimer (created in Plan 02 with is_low_time() method):

   In handle_authenticated_input, AFTER the timeout check and BEFORE processing input:
   ```rust
