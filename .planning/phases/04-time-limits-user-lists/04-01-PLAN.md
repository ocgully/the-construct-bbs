...
3. Add time banking columns to users table in schema.sql:
   Add these three columns directly to the existing CREATE TABLE IF NOT EXISTS users statement:
   - `daily_time_used INTEGER NOT NULL DEFAULT 0`
   - `banked_time INTEGER NOT NULL DEFAULT 0`
   - `last_daily_reset TEXT`

   This is a dev-stage project. The existing database must be deleted and recreated to pick up the new columns (SQLite's CREATE TABLE IF NOT EXISTS skips the statement entirely if the table exists). Delete `backend/bbs.db`, `backend/bbs.db-shm`, and `backend/bbs.db-wal` before testing.
