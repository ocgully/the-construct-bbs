...
1. Add `tokio-util` dependency to Cargo.toml:
   ...
   ```
   NOTE: The "rt" feature includes CancellationToken. If it doesn't compile, try features = ["sync"] or no features (CancellationToken is in the base crate since 0.7).
   ...
   impl SessionTimer {
       ...
       pub fn spawn(
           ...
           let handle_task = tokio::spawn(async move {
               run_timer(tx, remaining_minutes, handle, users_online, cancel_clone).await
           });
       ...
       loop {
           tokio::select! {
               _ = sec_ticker.tick() => {
                   ...
                   if remaining_secs <= 0 {
                       // Time expired -- send timeout signal
...
3. Add `pub mod timer;` to backend/src/connection/mod.rs.
  ...
  <done>
    SessionTimer struct exists with spawn(), cancel(), wait(), is_running() methods.
...
<task type="auto">
  <name>Task 2: Timeout Goodbye Screen and ANSI Status Bar Render Function</name>
  <files>
    backend/src/services/goodbye.rs
  </files>
  <action>
...
2. Add a `render_status_bar` function to backend/src/services/goodbye.rs (or a new file -- actually, add it to goodbye.rs since it's small and related to session display):

   Actually, the status bar will be rendered client-side (Plan 03 handles this). The server just sends JSON. No server-side ANSI status bar rendering needed -- the CONTEXT says "Server sends remaining time; client counts down." The status bar is a frontend concern.

   So Task 2 is just the timeout goodbye screen.
  </action>
  <verify>
    `cargo check` passes.
