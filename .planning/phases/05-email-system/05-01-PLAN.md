...
must_haves:
  ...
  artifacts:
    - path: "backend/src/db/messages.rs"
      provides: "Message struct, InboxEntry struct, and all CRUD functions"
      exports: ["Message", "InboxEntry", "create_message", "get_inbox_page", "get_inbox_count", "get_unread_count", "get_message_by_id", "mark_message_read", "delete_message", "get_sender_handles"]
...
2. Create backend/src/db/messages.rs with these structs and functions, following the string-based sqlx query pattern from db/user.rs:
...
- `InboxEntry` struct - a lightweight struct for inbox list queries that omits the body and recipient_id fields (we already know the recipient). Define it explicitly as:
...
- `get_inbox_page(pool, user_id, page, page_size) -> Result<Vec<InboxEntry>, sqlx::Error>`: SELECT id, sender_id, subject, sent_at, is_read FROM messages WHERE recipient_id = ? ORDER BY sent_at DESC LIMIT ? OFFSET ?. Returns Vec<InboxEntry> (not Message) to avoid loading message bodies for list views.
...
Run `cargo check` from backend directory to verify compilation. Run `cargo test` to verify existing tests still pass.
  ...
  <done>
messages.rs compiles with all CRUD functions including InboxEntry struct. Module registered in db/mod.rs. Schema includes messages table with foreign keys and indexes.
  </done>
</task>
...
5. InboxEntry struct has id, sender_id, subject, sent_at, is_read fields with FromRow derive
6. config.rs includes MailConfig with serde defaults
7. config.toml has [mail] section
</verification>

<success_criteria>
- Messages table DDL in schema.sql with CASCADE foreign keys
- All 8+ CRUD functions in db/messages.rs compile successfully
- InboxEntry struct defined with id (i64), sender_id (i64), subject (String), sent_at (String), is_read (i32) and derives Debug, Clone, FromRow
