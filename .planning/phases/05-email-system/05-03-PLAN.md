...
must_haves:
  ...
  artifacts:
    - path: "backend/src/websocket/session.rs"
      provides: "Sentinel service handlers for __mail_inbox__, __mail_read__, __mail_compose__ and login notification, plus mail command routing from menu"
  ...
  key_links:
    ...
    - from: "session.rs MenuAction::ExecuteCommand"
      to: "show_inbox"
      via: "mail command routing from menu dispatch"
      pattern: "\"mail\".*show_inbox"
---

<objective>
Wire mail functionality into the session lifecycle with sentinel service handlers, menu command routing, and login notification.

Purpose: This is the core integration plan that makes mail actually work. It connects the DB layer (Plan 01) and rendering/compose logic (Plan 02) to user input via the established sentinel service pattern, AND routes the "mail" menu command to the inbox.
Output: Working inbox navigation, message reading, compose/reply/delete flows, menu command routing, and login notification in session.rs.
</objective>
...
<task type="auto">
  <name>Task 1: Sentinel service handlers and mail command routing</name>
  <files>
    backend/src/websocket/session.rs
  </files>
  <action>
...
**Add "mail" command to MenuAction::ExecuteCommand match blocks:**

Session.rs has MenuAction::ExecuteCommand(cmd) match blocks that dispatch commands like "quit", "profile", "whos_online", etc. Add a "mail" arm to BOTH match blocks (there are two - one for single-char hotkey input and one for command dispatch). Following the exact same pattern as "profile" and "whos_online":

```rust
"mail" => { self.show_inbox().await; return; }
```

This routes the menu "M" hotkey (configured in config.toml Plan 05-04 as `command = "mail"`) to the show_inbox() helper method which sets current_service to "__mail_inbox__".
...
- The "mail" command routing in MenuAction::ExecuteCommand is CRITICAL - without it, the config.toml menu entry (Plan 05-04) will have no effect when the user presses "M" on the main menu
  </action>
  <verify>
Run `cargo check` to verify all handlers compile. Run `cargo test` to verify no regressions. Manually test by running the BBS and navigating to Mail from menu (will need Plan 04 for config.toml menu entry, but the handlers and command routing should compile).
  </verify>
  <done>
All three sentinel handlers (__mail_inbox__, __mail_read__, __mail_compose__) implemented in handle_authenticated_input. "mail" command added to both MenuAction::ExecuteCommand match blocks (same pattern as "profile", "whos_online"). Inbox shows paginated message list. Messages can be read, replied to, and deleted. Compose flow drives ComposeFlow state machine with async DB lookups. Self-mail and mailbox-full checks enforced.
  </done>
</task>
...
3. "mail" command routed in both MenuAction::ExecuteCommand match blocks to show_inbox()
4. Mail sentinel handlers route input correctly for inbox, read, and compose views
5. Compose flow handles To -> Subject -> Body with character-by-character input
6. Reply pre-fills quoted text and Re: subject
7. Delete removes message and returns to inbox
8. Self-mail blocked at compose time
9. Mailbox full check prevents sending
10. Login notification shows unread count
</verification>

<success_criteria>
- "mail" command dispatched in MenuAction::ExecuteCommand to show_inbox() (same pattern as "profile", "whos_online")
